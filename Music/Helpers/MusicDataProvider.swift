...
class MusicDataProvider {
    ...
    private func createArtist(data: Data, _ completion: @escaping (Artist?, Error?) -> Void) {
        ...
        guard let searchResults = try? JSONDecoder().decode(SearchResponse.self, from: data), searchResults.data.count > 0 else {
        ...
        getRequest(from: firstResult.tracklist) { (data, error) in
            ...
            guard let data = data, let songsArray = try? JSONDecoder().decode(SongsResponse.self, from: data) else {
                completion(self.artist, error)
                return
            }
